         ...
         if(isset($_view[0]) && isset($_view[1])){
            $output .= '<div>';
               ...
               try{
                  ...
                  if($view){
                     $args = array();
                     if (trim((string) $view_arg) !== '') {
                        $args = preg_split('/\//', trim((string) $view_arg), -1, PREG_SPLIT_NO_EMPTY);
                     }

                     if ($view->setDisplay($_view[1])) {
                        if (isset($view->display_handler->options['pager']['options']) && is_array($view->display_handler->options['pager']['options'])) {
                           $pager_options = &$view->display_handler->options['pager']['options'];
                           $pager_options['items_per_page'] = isset($pager_options['items_per_page']) && is_numeric($pager_options['items_per_page']) ? (int) $pager_options['items_per_page'] : 0;
                           $pager_options['offset'] = isset($pager_options['offset']) && is_numeric($pager_options['offset']) ? (int) $pager_options['offset'] : 0;
                        }
                     }

                     $v_output = $view->buildRenderable($_view[1], $args, FALSE);
                     if($v_output){
