...
class AutoEmbed {
    ...
    /**
     * Parses HTML tag attributes from a string like: key="value" key='value' key=value.
     * Returns an associative array of lowercased attribute names to their values.
     * @access private
     * @param string $text The attribute text to parse.
    ...
    private function parse_atts( $text ) {
            ...
            if ( preg_match_all( '/([a-zA-Z_:][-a-zA-Z0-9_:.]*)\s*=\s*(?:"([^"]*)"|\'([^\']*)\'|([^"\'>\s]+))/', $text, $matches, PREG_SET_ORDER ) ) {
    ...
    private function _parse_xml( $response_body ) {
            ...
            $dom = new DOMDocument();
            $success = $dom->loadXML( $response_body, LIBXML_NONET );
