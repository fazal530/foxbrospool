...
jQuery(document).ready(function($){
    function showGaviasSliderFallback($slider) {
      if (window.gaviasSliderLayerShowFallback) {
        window.gaviasSliderLayerShowFallback($slider);
      }
      $slider.trigger('gaviasSliderLayerFallback');
    }

    var $slider = jQuery("#slider-<?php print $_id ?>");
    $slider.data('gavias-slider-original-slide', $slider.find('> ul > li').first().clone(false));

    if (!jQuery.fn.revolution) {
      $slider.show();
      showGaviasSliderFallback($slider);
      return;
    }

    $slider.show().revolution({
      jsFileLocation:"<?php print base_path() . \Drupal::service('extension.path.resolver')->getPath('module', 'gavias_sliderlayer'); ?>/vendor/revolution/js/",
    ...
    window.setTimeout(function(){
      ...
      var hasLoader = $slider.find('.tp-loader:visible').length > 0;
      var hasVisibleMedia = $slider.find('.gva-slider-fallback-video:visible, .active-revslide video:visible, .active-revslide .tp-bgimg:visible').filter(function(){
        var $element = jQuery(this);
        var backgroundImage = $element.css('background-image');
        return $element.outerHeight() > 80 || (backgroundImage && backgroundImage !== 'none');
      }).length > 0;
      if (sliderHeight < 80 || hasLoader || !hasVisibleMedia) {
        showGaviasSliderFallback($slider);
      }
    }, 1800);
});
